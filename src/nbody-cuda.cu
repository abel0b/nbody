...
__global__ void UpdateParticle(const int nParticles,
                               struct ParticleType *const particle,
                               const float dt) {
    int i;
...
void MoveParticles(const int nParticles, struct ParticleType *const particle,
                   const float dt) {
    struct ParticleType *gpu_particle;
    cudaMalloc(&gpu_particle, sizeof(struct ParticleType) * nParticles);
    cudaMemcpy(gpu_particle, particle, sizeof(struct ParticleType) * nParticles,
               cudaMemcpyHostToDevice);

    UpdateParticle<<<(nParticles + 255) / 256, 256>>>(nParticles, gpu_particle,
                                                      dt);

    cudaMemcpy(particle, gpu_particle, sizeof(struct ParticleType) * nParticles,
...
int main(const int argc, const char **argv) {
    ...
    struct ParticleType *particle =
        (struct ParticleType *)malloc(nParticles * sizeof(struct ParticleType));
