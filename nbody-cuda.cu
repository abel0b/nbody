...
void MoveParticles(const int nParticles, struct ParticleType* const particle, const float dt) {
    ...
    cudaMemcpy(particle, gpu_particle, sizeof(struct ParticleType) * nParticles, cudaMemcpyHostToDevice);
  
    UpdateParticle<<<1,nParticles>>>(nParticles, gpu_particle, dt);
  
    cudaMemcpy(gpu_particle, particle, sizeof(struct ParticleType) * nParticles, cudaMemcpyDeviceToHost);
